...
DISTANCE_THRESHOLD = 0.7 #0.5 #0.2 	# (m)
VELOCITY = 0.18				# meters per second
FREQUENCY = 0.1 # Hz
...
def control(error):
	...
	prev_er = error
	int_prev = integral
	#print(f"derv {prev_er}")
	#print(f"int_prev {int_prev}")
...
def distance(angle_side, angle_lookahead, data, direction):
	...
	alpha = math.atan2(numerator_alpha, denominator_alpha)
	#alpha = math.atan(numerator_alpha / denominator_alpha)
	alpha_d = math.degrees(alpha)
	#dist_pred = FREQUENCY*vel
	
	distance_r = distance_b*math.cos(alpha) #+ dist_pred * math.sin(alpha)
	distance_r = abs(distance_b*math.cos(alpha)) #+ dist_pred * math.sin(alpha)
	
	# ---
	#print("dist_pred: %f" % dist_pred)
	print("Alpha: %f" % alpha_d)
	print (f"Distance from {direction} wall : %f" % distance_r)

	# Calculate error
	error = DISTANCE_THRESHOLD - distance_r
	
	print(f"Distance Error from {direction} wall: %f" % error)
